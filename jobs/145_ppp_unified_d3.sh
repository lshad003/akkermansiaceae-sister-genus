#!/bin/bash
#SBATCH --job-name=pppD3
#SBATCH --partition=epyc
#SBATCH --time=08:00:00
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --output=/bigdata/stajichlab/lshad003/ch3-chitin-evolution/logs/pppD3_%j.out
#SBATCH --error=/bigdata/stajichlab/lshad003/ch3-chitin-evolution/logs/pppD3_%j.err
module load diamond/2.2.6
CH3=/bigdata/stajichlab/lshad003/ch3-chitin-evolution
W=$CH3/results/ppp_unified_d3
mkdir -p $W
D=$(which diamond); echo "diamond: $D"; $D --version
M=$CH3/results/ppp_unified/manifest_v2.tsv
N=$(wc -l < $M); [ "$N" -eq 7631 ] || { echo "REFUSED: manifest has $N rows"; exit 1; }
: > $W/all.faa
...
man=[l.split("\t")[0].strip() for l in open(B+"/results/ppp_unified/manifest_v2.tsv") if l.strip()]
