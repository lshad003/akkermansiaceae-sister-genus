...
OUT = R + "/results/figures/Figure5_ppp_loss.pdf"
...
def load(p):
    ...
    for line in open(p):
        ...
        if len(f) >= 5:
            try:
                if float(f[2]) < 30.0: continue
            except ValueError:
                continue
            d[f[1].split("|")[0]].add(QN.get(f[0], f[0]))
    return d
ppp, ctl = load(W + "/ppp_trio.tsv"), load(W + "/control_gh20.tsv")
pgl, sgl = load(W + "/pgl.tsv"), load(W + "/sgl.tsv")

indb = set()
for line in open(R + "/results/ppp_unified/manifest.tsv"):
...
rows = ["zwf", "gnd", "opcA", "SGL-family", "pgl", "zwf-opcA adjacent", "GH20 control"]
...
for j, c in enumerate(keep):
    ...
    for i, r in enumerate(rows):
        if r == "GH20 control":
            ...
            k = sum(1 for g in gl if pgl.get(g))
        elif r == "SGL-family":
            k = sum(1 for g in gl if sgl.get(g))
        elif r == "opcA":
...
ax.set_title("Oxidative pentose phosphate pathway across Akkermansiaceae", fontsize=11)
